...
_APP_STYLE = """
<style>
/* Style is unchanged, hiding for brevity */
...
USER_INPUT_CHAR_LIMIT = 400 # <-- NEW: Character limit for collapsing user input
...
def _render_fir_analysis(res: dict):
    ...
    if res.get("sections"):
        for sec in res["sections"]:
            ...
            # Replace the nested expander with a collapsible section using st.markdown
...
def build_judgment_prompt(judgment_text: str):
    ...
    # <-- CHANGE 1: The description for "summary_en" now asks for an in-depth summary.
    json_schema = {
    ...
    # <-- CHANGE 2: The main instruction is updated to demand a more detailed analysis.
...
def _render_judgment_analysis(res: dict):
    ...
    # <-- CHANGE IS HERE
    # Replaced the st.expander with a regular subheader to avoid nesting.
...
with tab1:
    ...
    # <-- UPDATED: Chat history display loop with accordion logic
